 ...
 */
public class JsonParserJobs extends AsyncTask<String, Void, JSONResponseJobs>
{
    private Exception exception;
    private JobOfficeMainViewFragment jobOfficeMainViewFragment;

    public JsonParserJobs(JobOfficeMainViewFragment jobOfficeMainViewFragment)
    {
            this.jobOfficeMainViewFragment = jobOfficeMainViewFragment;
            final String url = "http://team-4.site11.com/index.php?api_key=4dbdb6281eda089ac926b65179ff4c29e417f6ec&action=getJobs";
            this.execute(url);

    }

    @Override
    protected JSONResponseJobs doInBackground(String... params) {
        try {
            ...
            return new Gson().fromJson(reader, JSONResponseJobs.class);
        } catch (Exception e) {
            this.exception = e;
            return null;
        }    }
    ...
    @Override
    protected void onPostExecute(JSONResponseJobs jsonResponse) {
        ...
        List<JSONGJobs> parsedResults = jsonResponse.data;

        // for each geoname let's toast the name
        for (JSONGJobs result : parsedResults) {
            //Toast.makeText(Welcome.this, result.DESCRIPTION, Toast.LENGTH_SHORT).show();
            jobOfficeMainViewFragment.updateList(result.NAME,result.DESCRIPTION,result.EMAIL);
